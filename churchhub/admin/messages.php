...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['subject'])) {
    ...
    if ($receiver_id === 0) {
        ...
        while ($m = $members->fetch_assoc()) {
            $stmt->bind_param("iiss", $admin_id, $m['member_id'], $subject, $body);
            $stmt->execute();
        }
        ...
        $stmt = $conn->prepare("
            ...
            VALUES (?, ?, 'member', ?, ?)
        ");
        $stmt->bind_param("iiss", $admin_id, $receiver_id, $subject, $body);
        $stmt->execute();
    ...
    $conn->query("
        ...
        VALUES ($admin_id, 'Send', 'messages', '$log_desc', '{$_SERVER['REMOTE_ADDR']}')
    ");
