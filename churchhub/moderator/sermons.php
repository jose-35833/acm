...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($sermon_id) {
        ...
        $conn->query("INSERT INTO admin_logs (admin_id, action, table_name, record_id, description)
                      VALUES ($moderator_id, 'Update', 'sermons', $sermon_id, 'Updated sermon: $title')");
    } else {
        // Add new sermon
        $stmt = $conn->prepare("INSERT INTO sermons (title, description, video_link, posted_by) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("sssi", $title, $description, $video_link, $moderator_id);
        $stmt->execute();

        $new_id = $conn->insert_id;
        $conn->query("INSERT INTO admin_logs (admin_id, action, table_name, record_id, description)
                      VALUES ($moderator_id, 'Add', 'sermons', $new_id, 'Added new sermon: $title')");
    }

    echo "<script>alert('Sermon saved successfully');window.location='sermons.php';</script>";
}
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("INSERT INTO admin_logs (admin_id, action, table_name, record_id, description)
                  VALUES ($moderator_id, 'Delete', 'sermons', $delete_id, 'Deleted a sermon record')");

    echo "<script>alert('Sermon deleted successfully');window.location='sermons.php';</script>";
}
?>
