...
if (isset($_POST['action_type']) && isset($_POST['member_id'])) {
    ...
    if ($action === 'promote') {
        ...
        $conn->query("
            ...
            VALUES ($moderator_id, 'Promote', 'members', 'Promoted member ID $member_id to admin', '{$_SERVER['REMOTE_ADDR']}')
        ");
        ...
        $conn->query("
            INSERT INTO admin_logs (admin_id, action, table_name, description, ip_address)
            VALUES ($moderator_id, 'Demote', 'members', 'Demoted admin ID $member_id back to member', '{$_SERVER['REMOTE_ADDR']}')
        ");
